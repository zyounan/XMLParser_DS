...
static int center_x(int width) { return (COLS - width) / 2; }

static int center_y(int height) { return (LINES - height) / 2; }

static WINDOW *create_prompt(const char *message, int height, int width) {
    WINDOW *prompt =
        subwin(stdscr, height, width, center_y(height), center_x(width));
    werase(prompt);
...
void prompt_string(const char *message, char *name, int size) {
    echo();
    WINDOW *prompt =
        create_prompt(message, PROMPT_STRING_LINES, PROMPT_STRING_COLS);
